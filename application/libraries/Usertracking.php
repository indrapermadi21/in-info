...
class Usertracking
{
  ...
  private $needed_fields = array(array('name' => 'id', 'type' => 'int', 'primary_key' => 1, 'forge_type' => 'int', 'forge_auto_increment' => TRUE),
														...
														array('name' => 'request_stream_data', 'type' => 'string', 'forge_type' => 'text'));
  ...
  public function initialize()
  {
    ...
    /*
	if ( ! $this->CI->config->load('usertracking_config', TRUE) OR $this->CI->config->item('usertracking') === FALSE)
    {
  ...
  public function track_this()
  {
    ...
    //get the data
    $input_data = array();
    $input_data['session_id'] = $this->CI->session->userdata('logged_in')["sess_id"];
    ...
    $input_data['request_stream_data'] = "GET --> " . var_export($this->CI->input->get(NULL, TRUE), TRUE) . ", POST --> " . var_export($this->CI->input->post(NULL, TRUE), TRUE);
	...
	//force-value
	$input_data['user_identifier'] = $this->CI->session->userdata('logged_in')["username"];
	
    //Add it to the database
	$this->CI->load->database();
    
	$this->CI->db->set('session_id', strlen($input_data['session_id']) > 0 ? $input_data['session_id']: "[unauthenticated]");
	$this->CI->db->set('user_identifier', strlen($input_data['user_identifier']) > 0 ? $input_data['user_identifier']: "[unauthenticated]");
    $this->CI->db->set('request_uri', $input_data['request_uri']);
    $this->CI->db->set('timestamp', $input_data['timestamp']);
    $this->CI->db->set('client_ip', $input_data['client_ip']);
    $this->CI->db->set('client_user_agent', $input_data['client_user_agent']);
    $this->CI->db->set('referer_page', $input_data['referer_page']);
    $this->CI->db->set('request_stream_data', 'ENCODE("' . htmlspecialchars($input_data['request_stream_data']) . '", "insan123")', FALSE);
	
	
    $result = $this->CI->db->insert('tb_usertracking');
  ...
  private function check_database()
  {
    ...
    if ($this->CI->db->table_exists('tb_usertracking'))
    {
      //if the table exists, check to see if the columns are setup correctly
      $fields = $this->CI->db->field_data('tb_usertracking');
      ...
      if ($num_matched < count($this->needed_fields) && $this->configuration['auto_fix_db'] === TRUE)
      {
        ...
        $this->CI->dbforge->rename_table($db_prefix . 'tb_usertracking', 'tb_usertracking_backup_' . time());
  ...
  private function build_database_table()
  {
    ...
    $this->CI->dbforge->create_table('tb_usertracking');
    return TRUE;
  }
