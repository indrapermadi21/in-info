...
class Ion_auth_model extends CI_Model
{
	...
	function insertuser($ip_addr,$id_perusahaan,$id_candidate,$password,$email,$created_on,$firsname,$lastname,$telepon){
		$hash_pass = $this->hash_password($password);
		$sql = "INSERT INTO users (ip_address,id_perusahaan,id_candidate,id_employee,username,password,salt,email,activation_code,forgotten_password_code,forgotten_password_time,
							remember_code,created_on,last_login,active,first_name,last_name,phone) 
				VALUES(?,?,?,0,'',?,'',?,'','','','',?,'',1,?,?,?)";
		$query = $this->db->query($sql,array($ip_addr,$id_perusahaan,$id_candidate,$hash_pass,$email,$created_on,$firsname,$lastname,$telepon));
	...
	function insertcandidate($nama,$telepon,$email){
		$sql = "INSERT INTO t_candidate_employee (nama_lengkap,hp,email) 
				VALUES(?,?,?)";
		$query = $this->db->query($sql,array($nama,$telepon,$email));
		if($this->db->affected_rows() > 0){
			$data=array(
				  'code' => '1'
				);
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$data = array(
		    ...
		    'password'   => $password,
		    'email'      => $email,
		    // 'ip_address' => $ip_address, // ip address di comment tidak dibutuhkan  untuk sekarang
		    'created_on' => time(),
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, foto, first_name,id_job,nik')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'email'                => $user->email,
		    'nama'                 => array($user->first_name),
		    'user_id'              => $user->id, //everyone likes to overwrite id so we'll use user_id
		    // 'emp_id'               => $user->id_employee,
		    'id_job'                => $user->id_job,
		    //'namkar'               => $user->nama_karyawan,
		    'foto'                 => $user->foto,
		    //'id_addr'              => $user->ip_address,
	...
	**/
	...
	public function get_filtered_groups($creating_group = GROUP_SUPER_ADMIN) {
